/*
  ...
  - Background (image_b701a4.jpg) cover-fit (no stretch)
  - Left donut (brown palette) + right animated bars
  - Two-way sync: bar <-> donut (hover + click)
  - Score Detail: centered modal pops & fades IN on selection (click), never on hover
  - Close via × button, ESC, or backdrop click
  - Enterprise-clean styling (no glow fonts)
*/
...
let SEA_DATA = [
  ...
  // ---------- Palette ----------
  const colors = {
    barBase: 48,                    // neutral dark
    gold: [255, 185, 95],           // active highlight
    textPrimary: [245,245,245],
    textSecondary: [210,210,210],
    textMuted: [180,180,180]
  };
  const BROWNS_HEX = ["#3E2723","#4E342E","#5D4037","#6D4C41","#795548","#8D6E63","#A1887F","#BCAAA4","#D7CCC8"];
  ...
  let bgImg;
  let barStates = [];
  let selectedIndex = -1;        // clicked selection (controls modal)
  let hoverBarIndex = -1;        // bar hover
  let hoverDonutIndex = -1;      // donut hover
  let startTime = 0;
  const ENTER_STAGGER = 70, ENTER_DUR = 700;
  
  // Detail modal animation
  let detailAlpha = 0;           // 0..1 opacity
  let detailScale = 0.96;        // pop scale (0.96 → 1.0)
  let detailOpenMs = 0;          // timestamp when selection opened
  const DETAIL_FADE_SPEED = 0.2;
  const DETAIL_POP_DURATION = 260; // ms
  
  // Card geometry for hit testing (updated each draw)
  const cardRect = { x:0,y:0,w:720,h:340 }; // (h a bit taller for centered title layout)
  let closeBtn = { x:0,y:0,w:28,h:28 };
  
  // ---------- Preload ----------
  function preload(){ bgImg = loadImage("image_b701a4.jpg"); }
  ...
  function draw(){
    ...
    // Hover priority for visuals (no selection side effects)
    ...
    // Subtle hint to click when nothing selected
    if (selectedIndex < 0){
      drawClickHint();
    }
  
    // Detail modal: fade & pop only when selected
  ...
  function drawBars(visualHoverIndex){
    ...
    for (let i=0;i<SEA_DATA.length;i++){
      ...
      // bar body
      if (isSel || isHov){ noStroke(); fill(colors.gold[0], colors.gold[1], colors.gold[2], alpha); }
  ...
  function drawDonutSynced(visualHoverIndex){
    ...
    // active index for highlight: selection > hover (bar or donut)
    ...
    for (let i=0;i<SEA_DATA.length;i++){
      ...
      let col = color(BROWNS_HEX[i % BROWNS_HEX.length]);
      let alpha = (activeIdx >= 0 && i !== activeIdx) ? 110 : 225;
      let expand = 0;
  
      if (i === activeIdx){
        col = color(colors.gold[0], colors.gold[1], colors.gold[2]); // same gold as bars
        alpha = 255;
  ...
  // ---------- Score Detail (CENTERED MODAL, pop+fade on selection only) ----------
  function drawScoreDetailCentered(idx, alpha, scaleAmt){
    ...
    // score badge (centered under title)
    ...
    // right column (neat key/value list)
  ...
  function drawScoreBadge(x,y,w,h,score,alpha){
    ...
    stroke(colors.gold[0], colors.gold[1], colors.gold[2], 140*alpha); noFill(); rect(x,y,w,h,999);
  ...
  // Small bottom hint when nothing selected
  ...
  function mousePressed(){
    // If modal open: detect close button or backdrop click
    if (detailAlpha > 0.9 && selectedIndex >= 0){
      ...
      // Backdrop click (outside card bounds)
      const insideCard =
      ...
      // If click is inside the card but not on close, do nothing.
      return;
    }
    ...
    if (bi >= 0){
      selectedIndex = (selectedIndex === bi) ? -1 : bi;
      if (selectedIndex >= 0) detailOpenMs = millis();
      return;
    }
    ...
    if (di >= 0){
      selectedIndex = (selectedIndex === di) ? -1 : di;
      if (selectedIndex >= 0) detailOpenMs = millis();
      return;
    }
  }
  ...
  function keyPressed(){
    if (keyCode === ESCAPE && selectedIndex >= 0){
      selectedIndex = -1;
    }
  }
